import { defaultOverride, defaultUpdateOptions } from './chunk-NGVXFKGX.mjs';
import { Web3Provider, StaticJsonRpcProvider, JsonRpcProvider, UrlJsonRpcProvider, JsonRpcBatchProvider, WebSocketProvider, Provider } from '@ethersproject/providers';
...
var isEthersProvider = (providerBase) => {
  if (providerBase == null)
    return false;
  return providerBase instanceof Web3Provider || providerBase instanceof StaticJsonRpcProvider || providerBase instanceof JsonRpcProvider || providerBase instanceof UrlJsonRpcProvider || providerBase instanceof JsonRpcBatchProvider || providerBase instanceof WebSocketProvider;
};
...
var mergeDefaultOverride = (...overrides) => {
  const defaultOptions = defaultOverride();
...
var mergeDefaultUpdateOptions = (...overrides) => {
  ...
  const defaultOptions = defaultUpdateOptions();
  if ((overrides == null ? void 0 : overrides.length) > 0) {
...
var providerKey = (providerOrSigner) => {
  ...
  if (Provider.isProvider(providerOrSigner)) {
    ...
    if (provider && (provider == null ? void 0 : provider.network) && Signer.isSigner(providerOrSigner)) {
      return {
...
var isProvider = (providerOrSigner) => {
  ...
  if (providerOrSigner instanceof JsonRpcProvider || providerOrSigner instanceof Web3Provider || providerOrSigner instanceof StaticJsonRpcProvider) {
    return true;
...
export { adaptorKey, asEthersAdaptor, asyncForEach, asyncSome, checkUpdateOptions, contractFuncKey, contractKey, ethersOverride, eventKey, invalidateCache, isAdaptorEqual, isEthersProvider, isProvider, isValidEthersAdaptor, isValidEthersContext, logQueryCache, mergeDefaultOverride, mergeDefaultUpdateOptions, parseProviderOrSigner, processQueryOptions, providerKey, setContextOverride, signerHasNetwork };
//# sourceMappingURL=chunk-YI4OAWCG.mjs.map