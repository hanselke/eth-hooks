...
function mockifyContractFactory(vm, contractName, factory) {
    ...
    factory.deploy = async (...args) => {
        ...
        const readableStorage = new readable_storage_logic_1.ReadableStorageLogic(await storage_1.getStorageLayout(contractName), vm.getManager(), mock.address);
        ...
        mock.wallet = await utils_1.impersonate(mock.address);
        return mock;
    };
        ...
        return (values) => {
            ...
            try {
                ...
                try {
                    ...
                    if (utils_1.isPojo(values)) {
                        return contractInterface.encodeFunctionResult(fnFragment, utils_1.convertPojoToStruct(values, fnFragment));
                    }
                    throw err;
                }
            }
...
function parseAndFilterBeforeMessages(messages$, contractInterface, contractAddress, sighash) {
    ...
    }), operators_1.map((message) => parseMessage(message, contractInterface, sighash)), operators_1.share());
}
...
function parseMessage(message, contractInterface, sighash) {
    return {
        ...
        target: utils_1.fromFancyAddress(message.delegatecall ? message.codeAddress : message.to),
        delegatedFrom: message.delegatecall ? utils_1.fromFancyAddress(message.to) : undefined,
    };
}
