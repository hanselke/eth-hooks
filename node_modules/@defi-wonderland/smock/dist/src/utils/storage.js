...
function computeStorageSlots(storageLayout, variables = {}) {
    ...
    slots = slots.reduce((prevSlots, slot) => {
        ...
        if (prevSlot === undefined) {
            ...
            let mergedVal = '0x';
            const valA = utils_1.remove0x(slot.val);
            const valB = utils_1.remove0x(prevSlot.val);
...
function encodeVariable(variable, storageObj, storageTypes, nestedSlotOffset = 0, baseSlotKey) {
    let slotKey = '0x' +
        utils_1.remove0x(ethers_1.BigNumber.from(baseSlotKey || nestedSlotOffset)
    ...
    if (variableType.encoding === 'inplace') {
        if (variableType.label === 'address' || variableType.label.startsWith('contract')) {
            ...
            return [
                {
                    key: slotKey,
                    val: padBytesHexSlotValue(utils_1.remove0x(variable).padEnd(parseInt(variableType.numberOfBytes, 10) * 2, '0'), storageObj.offset),
                },
            ];
        }
        else if (variableType.label.startsWith('uint') || variableType.label.startsWith('int')) {
            if (utils_1.remove0x(ethers_1.BigNumber.from(variable).toHexString()).length / 2 > parseInt(variableType.numberOfBytes, 10)) {
        ...
        const bytes = storageObj.type === 'string' ? ethers_1.ethers.utils.toUtf8Bytes(variable) : utils_1.fromHexString(variable);
        if (bytes.length < 32) {
            return [
                {
                    key: slotKey,
        ...
        for (const [varName, varVal] of Object.entries(variable)) {
            ...
            if (variableType.key.startsWith('t_uint')) {
                ...
                key = '0x' + utils_1.remove0x(varName).padEnd(64, '0');
            }
            else {
                key = varName;
            }
            const prevBaseSlotKey = baseSlotKey || padNumHexSlotValue(storageObj.slot, 0);
            const nextBaseSlotKey = ethers_1.ethers.utils.keccak256(padNumHexSlotValue(key, 0) + utils_1.remove0x(prevBaseSlotKey));
...
async function getVariableStorageSlots(storageLayout, variableName, vmManager, contractAddress, mappingKey, baseSlotKey, storageType) {
    ...
    let key = baseSlotKey ||
        '0x' +
            utils_1.remove0x(ethers_1.BigNumber.from(0)
                .add(ethers_1.BigNumber.from(parseInt(storageObj.slot, 10)))
                .toHexString()).padStart(64, '0');
...
function getStructTypeStorageSlots(storageLayout, key, storageObjectType, storageObj) {
    ...
    slotKeysTypes = slotKeysTypes.concat(storageObjectType.members.map((member) => ({
        key: '0x' + utils_1.remove0x(ethers_1.BigNumber.from(key).add(ethers_1.BigNumber.from(member.slot)).toHexString()).padStart(64, '0'),
...
async function getBytesTypeStorageSlots(vmManager, contractAddress, storageObjectType, storageObj, key) {
    let slotKeysTypes = [];
    const bytesValue = utils_1.toHexString(await vmManager.getContractStorage(utils_1.toFancyAddress(contractAddress), utils_1.fromHexString(key)));
...
async function getMappingTypeStorageSlots(storageLayout, variableName, vmManager, contractAddress, key, storageObjectType, mappingKey, baseSlotKey) {
    ...
    if (storageObjectType.key.startsWith('t_uint')) {
        ...
        mappKey = '0x' + utils_1.remove0x(mappingKey[0]).padEnd(64, '0');
    }
    else {
        mappKey = mappingKey[0];
    }
    const prevBaseSlotKey = baseSlotKey || key;
    let nextSlotKey = ethers_1.ethers.utils.keccak256(padNumHexSlotValue(mappKey, 0) + utils_1.remove0x(prevBaseSlotKey));
    let slotKeysTypes = [];
    mappingKey.shift();
...
async function getDynamicArrayTypeStorageSlots(vmManager, contractAddress, storageObjectType, key) {
    let slotKeysTypes = [];
    let arrayLength = parseInt(utils_1.toHexString(await vmManager.getContractStorage(utils_1.toFancyAddress(contractAddress), utils_1.fromHexString(key))), 16);
    ...
    if (slotValueTypePairs[0].type.encoding === 'inplace') {
        if (slotValueTypePairs[0].type.label === 'address' || slotValueTypePairs[0].type.label.startsWith('contract')) {
            ...
            if (intHex.slice(0, 1) === 'f') {
                intHex = utils_1.fromHexString('0x' + intHex);
                const mask = utils_1.fromHexString('0xFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFF');
                intHex = -ethers_1.BigNumber.from(utils_1.toHexString(utils_1.xor(intHex, mask))).add(ethers_1.BigNumber.from(1));
            }
            result = intHex;
        }
        ...
        for (const slotKeyPair of slotValueTypePairs) {
            ...
            if (slotKeyPair.length < 32) {
                ...
                result = utils_1.remove0x(result);
